...
def maybe(*args):
    """A ref that exists only as a remote-tracking ref records `null` locally
    rather than failing: this pass reviews a branch it never checked out."""
    try:
        return git(*args)
    except subprocess.CalledProcessError:
        return None
BRANCH = 'candidate/phase2-gate-c2-w01-correction-20260912'
pinned = {
    'review/phase2-gate-c2-decay-write-resolution-independent-20260912': 'e39690dc51b63fa09fe7c3f30ebd48aa2c16686f',
    'candidate/phase2-gate-c2-decay-write-resolution-20260912': 'cc3dc70182b428f7cbc953ae722f85b39d407dfa',
...
for name, expected in pinned.items():
    row = dict(expected=expected, local=maybe('rev-parse', 'refs/heads/' + name),
               tracking=git('rev-parse', 'refs/remotes/origin/' + name), live=live[name])
    row['equal'] = all(row[k] == expected for k in ['tracking', 'live']) and \
        row['local'] in (expected, None)
