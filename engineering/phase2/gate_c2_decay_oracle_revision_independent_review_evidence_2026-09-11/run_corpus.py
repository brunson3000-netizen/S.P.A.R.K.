...
output is otherwise exactly what cargo printed. The runner's own exit status is zero after capture. The cargo exit status is
recorded in the JSON line it prints; a failing corpus is evidence, not semantic
success. Inspect the log and the recorded cargo status separately.
"""
