"""Fresh validation for the Gate C2 decay-write resolution pass. It reruns every
...
checks = [
    ...
    ('whitespace-candidate-range', f'git diff --check {BASE} HEAD'),
    ('whitespace-production-range', f'git diff --check {PROD} HEAD'),
    ('whitespace-review-range', f'git diff --check {REVIEW} HEAD'),
]
