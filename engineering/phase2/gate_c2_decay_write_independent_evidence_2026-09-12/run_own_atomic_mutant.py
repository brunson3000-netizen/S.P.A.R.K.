...
mutants=[dict(id='OWN-family-mixture-commits',test='own_watchers_and_atomic_refusal',patches=[('crates/spark-engine/src/effects.rs', '        } else {\n            return Err(WaveRejection::FamilyMixture { definition, scope });\n        };', '        } else {\n            pre(&definition, &scope).unwrap_or(0)\n        };')])]
...
with tempfile.TemporaryDirectory(prefix='spark-c2-own-mutants-') as tmp:
 ...
 for m in mutants:
  ...
  (out/'own-atomic-mutations.json').write_text(json.dumps(result,indent=2)+'\n'); print(entry['id'],entry['baseline_pass'],entry['killed'],flush=True)
