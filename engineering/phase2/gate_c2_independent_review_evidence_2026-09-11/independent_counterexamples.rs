...
fn derived_identity_binds_every_changed_eligibility_input() {
    ...
    let cohort=report.cohort_identity.as_ref().unwrap();
    let actual=e.obligations().keys().next().and_then(|k|e.obligations().get(k)).unwrap().sole_record().unwrap().creator_emission_identity();
    assert_eq!(actual,&schedule_identity(cohort,&stress,&watch,&artifact),"negative control: implementation binds watched target alone");
...
fn decay_preserves_elapsed_cadence_remainder() {
    fn run(times: &[u64]) -> i64 {
        ...
        let decay=work_rule("rule.decay","work.decay",vec![emit("x","state.stress",Update::Decay { toward:lit(0),rate:lit(10),cadence:10 })]);
        let mut e=f.engine(budgets(10),vec![setup,decay],times.iter().map(|t|initial("rule.decay",&actor("a"),*t,"work.decay")).collect());
        e.process(&command("cmd.setup",0,1,"cmd.setup",actor("a")));
...
#[test]
fn refusal_type_must_not_be_constructible_externally() {
    // This line compiling is the counterexample to Rev2 oracle §10.
    let forged=spark_engine::request::FinalizationRefusal::WrongProfile;
    println!("external construction succeeds: {forged:?}");
}
...
fn new_decay_rate_must_not_apply_before_activation_barrier() {
    let mut f=standard();
    let setup=on_command("rule.setup","cmd.setup",vec![emit("x","state.stress",Update::Assign(lit(100)))]);
    let decay=work_rule("rule.decay","work.decay",vec![emit("x","state.stress",Update::Decay {
        toward:lit(0), rate:Expr::Input(Input::Config { key:def("tune.rate") }), cadence:10 })]);
    let mut g=f.genesis(budgets(10),vec![setup,decay],vec![initial("rule.decay",&actor("a"),60,"work.decay")]);
