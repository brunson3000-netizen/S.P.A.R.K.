...
#[test] fn luna_extreme_assignments_and_zero_cadence_observation(){
    for value in [i64::MAX, i64::MIN, 0, -1, 1] {
        let mut r=R::new("state.pressure",vec![wide(),sub(1)],10,10,1,&[],None);
        let c=r.cmd(0,"cmd.set",value); let (o,e)=r.execute(c);
        println!("LUNA extreme value={value} outcome={o:?} effects={:?} cell={:?}",e,r.read("state.pressure"));
    }
    let mut z=R::new("state.pressure",vec![wide(),sub(1)],0,1,1,&[1],None);
    z.send(0,"cmd.set",2); z.go(1); println!("LUNA zero-rate cell={:?}",z.read("state.pressure"));
}

#[test] fn luna_seeded_deterministic_sequences(){
    for mut seed in [1u64, 17, 0x5eed, u64::MAX-1] {
        let mut r=R::new("state.pressure",vec![half(),sub(-3),wide()],7,5,2,&[20,40,60],None);
        r.send(0,"cmd.set",50);
        for i in 0..12u64 {
            seed = seed.wrapping_mul(6364136223846793005).wrapping_add(1);
            let t = 2 + i*3 + (seed % 3);
            let v = (seed as i64 % 41) - 20;
            if i % 3 == 0 { r.send(t,"cmd.set",v); } else { r.send(t,"cmd.body",0); }
        }
        r.replay();
        println!("LUNA seeded sequence final={:?}",r.read("state.pressure"));
    }
}
