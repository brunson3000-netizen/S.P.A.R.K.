"""Compiled wrong-implementation controls for the Gate C2 C2W-01 correction pass.
...
The first sixteen are carried forward with byte-identical patches from the preceding
passes and re-verified against this tree: `decay_walk`, `baseline_of`, `settled`,
`settlement_operation` and `effects::reduce` are all reused unchanged, so every prior
discriminator must still kill them. The superseded elapsed-since-write mutant
(`C2-05-relative-steps`) and the settlement-operation mutant (`RES-current-epoch-only`)
are additionally required to be killed by the new composition vectors.

One prior mutant is deliberately **not** carried forward. `RES-settle-composed-body`
patched `let mut v = i128::from(current.unwrap_or(0));`, the line this correction
replaces, and its "wrong implementation" — settling every composed body — is now the
distinct over-broad mutant `W01-settle-every-body` below. Its patch can no longer match,
so re-running it unchanged would be a compile failure relabelled as a kill. It is
replaced, not quietly dropped.

The last four are new, and are exactly the wrong implementations of the corrected rule:
forfeiting the debt for any body with a transform stage (the C2W-01 regression itself),
settling a body that declares a decay stage (the double charge), settling every composed
body including transform-only ones (breaking the accepted standalone boundary), and
letting earlier decay hold a declared replacement up.
...
- `survives`: prior tests that PASS on the mutant, where a review identified such a gap.
...
MUTANTS = [
    ...
    dict(id='C2-05-relative-steps',
         ...
         kills=[(D, 'c2r_02_03_grid_matches_the_reference_model_under_every_partition'),
                ...
                ('phase2_decay_write_composition', 'an_additive_composition_settles_before_its_declared_stages')],
         survives=[]),
...
RESOLUTION_MUTANTS = [
    ...
    dict(id='RES-current-epoch-only',
         ...
         kills=[REMOVE, ('phase2_decay_write_composition',
                        'a_composed_write_settles_the_closing_segment_while_the_operation_is_removed')],
         survives=[]),
...
C = 'phase2_decay_write_composition'
COMPOSED = (C, 'an_additive_composition_settles_before_its_declared_stages')
CATCHUP = (C, 'a_composed_catch_up_equals_an_explicit_pre_evaluation')
NONIDENTITY = (C, 'nonidentity_transforms_resolve_from_the_settled_sum')
SUCCESSIVE = (C, 'successive_composed_writes_never_charge_a_grid_point_twice')
DECAY_ORDER = (C, 'a_body_that_declares_a_decay_stage_keeps_its_order_and_charges_once')
TRANSFORM_ONLY = (C, 'transform_only_paths_are_not_additive_events')
REPLACEMENT_IN_BODY = (C, 'a_declared_replacement_inside_a_body_wins_in_either_stage_order')
COMPOSED_REMOVED = (C, 'a_composed_write_settles_the_closing_segment_while_the_operation_is_removed')
COMPOSED_BARRIER = (C, 'a_composed_write_at_a_barrier_bills_the_closing_endpoint_once')
COMPOSED_REFUSAL = (C, 'a_refused_composed_wave_settles_nothing')
COMPOSED_WATCHER = (C, 'a_composed_write_is_one_committed_transition_a_watcher_can_see')

START = ("        let start = if is_additive_composition {\n"
...
W01_MUTANTS = [
    dict(id='W01-composed-lost-debt',
         wrong='the C2W-01 regression: a composed body folds from the raw committed value, so an additive stage finished by a transform forfeits every grid step that ended before it',
         patches=[(ENGINE, START,
                   "        let _ = is_additive_composition;\n        let start = current;")],
         kills=[COMPOSED, CATCHUP, NONIDENTITY, SUCCESSIVE, COMPOSED_REMOVED, COMPOSED_BARRIER,
                COMPOSED_REFUSAL, COMPOSED_WATCHER],
         survives=[]),
    dict(id='W01-settle-decay-body',
         wrong='a body that declares a decay stage is also settled, so that stage walks the same endpoints a second time',
         patches=[(ENGINE,
                   "        let is_additive_composition = !declares_decay\n            && ops",
                   "        let is_additive_composition = declares_decay\n            || ops")],
         kills=[DECAY_ORDER],
         survives=[]),
    dict(id='W01-settle-every-body',
         wrong='every composed body is settled, including one of transform stages alone, breaking the accepted standalone Scale/Clamp boundary',
         patches=[(ENGINE,
                   "                .any(|o| matches!(o.update, Update::Add(_) | Update::Subtract(_)));",
                   "                .any(|_o| true);")],
         kills=[TRANSFORM_ONLY],
         survives=[]),
    dict(id='W01-replacement-held-up-by-decay',
         wrong="a declared replacement inside a body cannot lower the value below the settled starting value (a wrong reading of \"earlier decay must not reduce that replacement\")",
         patches=[(ENGINE,
                   "                Update::Assign(x) => i128::from(self.expr(x, subject, ctx)?),\n                Update::Scale(f) => scale_wide(v, f.raw())?,",
                   "                Update::Assign(x) => i128::from(self.expr(x, subject, ctx)?).max(v),\n                Update::Scale(f) => scale_wide(v, f.raw())?,")],
         kills=[REPLACEMENT_IN_BODY],
         survives=[]),
]
MUTANTS = MUTANTS + W01_MUTANTS
...
with tempfile.TemporaryDirectory(prefix='spark-c2-w01-mutants-') as tmp:
    tmp = Path(tmp)
    ...
    for t in tests:
        res, baseline_output = run_test(base, *t)
        (out / ('baseline-' + t[1] + '.txt')).write_text(normalize('$ cargo test --offline -p spark-testkit --all-features --test ' + t[0] + ' -- --exact ' + t[1] + '\n' + baseline_output))
